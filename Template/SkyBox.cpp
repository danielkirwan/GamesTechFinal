...
bool CSkybox::Create(const char *bmpDirectory, const float fSize)
{
	// Load textures
	CTexture texture;	
	//Changed the skybox texture to that of stars in space
	texture.Load("Resources\\Textures\\spaceBack1.bmp", true);
	m_textureIDs[SKYBOX_BACK_ID] = texture.m_textureID;

	texture.Load("Resources\\Textures\\spaceFront1.bmp", true);
	m_textureIDs[SKYBOX_FRONT_ID] = texture.m_textureID;

	texture.Load("Resources\\Textures\\spaceBottom1.bmp", true);
	m_textureIDs[SKYBOX_BOTTOM_ID] = texture.m_textureID;

	texture.Load("Resources\\Textures\\spaceTop1.bmp", true);
	m_textureIDs[SKYBOX_TOP_ID] = texture.m_textureID;

	texture.Load("Resources\\Textures\\spaceLeft1.bmp", true);
	m_textureIDs[SKYBOX_LEFT_ID] = texture.m_textureID;

	//Need to adjust this image as there is some white on the left side
	texture.Load("Resources\\Textures\\spaceRight1.bmp", true);
